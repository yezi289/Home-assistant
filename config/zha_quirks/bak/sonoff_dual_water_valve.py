...
class SingleIrrigationState:
    ...
    total_duration_min: int = SINGLE_IRRIGATION_DEFAULT_AMOUNT
        ...
        **kwargs,
        ...
        if attr_ids.intersection(
            ...
            config_result = await self.endpoint.sonoff_cluster.write_attributes(
                {SonoffWaterValveCluster.AttributeDefs.single_irrigation_set.id: zcl_array}
            )
...
def add_irrigation_plan_config_entities(builder: QuirkBuilder) -> QuirkBuilder:
    """Add config entities for the aggregate irrigation  plan attribute 0xFBFD."""
    ...
    return (
        builder.adds(SonoffIrrigationPlanConfigCluster)
         .number(
            ...
            fallback_name="日程 irrigation plan index",
        )
        .number(
            ...
            fallback_name="日程 effective year",
        )
        .number(
            ...
            fallback_name="日程 effective month",
        )
        .number(
            ...
            fallback_name="日程 effective day",
        )
        .enum(
            SonoffIrrigationPlanConfigCluster.AttributeDefs.amount_unit.name,
            IrrigationAmountUnit,
            SonoffIrrigationPlanConfigCluster.cluster_id,
            translation_key="schedule_irrigation_amount_unit",
            fallback_name="日程 irrigation amount unit",
        )
        .enum(
...
def add_common_entities(builder: QuirkBuilder) -> QuirkBuilder:
    ...
    return (
        add_single_irrigation_config_entities(
            add_irrigation_plan_config_entities(builder.replaces(SonoffWaterValveCluster))
        )
        .enum(
            SonoffSingleIrrigationConfigCluster.AttributeDefs.amount_unit.name,
            IrrigationAmountUnit,
